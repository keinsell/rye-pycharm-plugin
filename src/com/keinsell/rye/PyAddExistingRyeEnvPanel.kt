...
class PyAddExistingRyeEnvPanel(private val project: Project?,
    ...
    init {
        ...
        addInterpretersAsync(sdkComboBox) {
            ...
            val moduleSdks = allModules(project).parallelStream().flatMap { module ->
                val sdks = detectRyeEnvs(module, existingSdkPaths, module.basePath)
                    .filterNot { it.isAssociatedWithAnotherModule(module) }
                sdks.forEach { sdkToModule.putIfAbsent(it.name, module) }
                sdks.stream()
            }.toList()
            val rootSdks = detectRyeEnvs(module, existingSdkPaths, project?.basePath ?: newProjectPath)
                .filterNot { it.isAssociatedWithAnotherModule(module) }
    ...
    override fun getOrCreateSdk(): Sdk? {
        return when (val sdk = sdkComboBox.selectedSdk) {
            is PyDetectedSdk -> {
                ...
                setupRyeSdkUnderProgress(project, mappedModule, existingSdks, newProjectPath,
